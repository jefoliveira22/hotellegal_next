...
export default function TabelaCamareiro(props) {
    ...
    return (
        <Container className='mb-5'>
            <Navbar expand="lg">
                ...
                <Navbar.Collapse id="navbarScroll">
                    <Nav className="me-auto my-2 my-lg-0" style={{ maxHeight: '100px' }} navbarScroll>
                        <Button variant="outline-success" onClick={() => { props.mudaTela(CAMAREIRO.cadastracam) }}>
            ...
            <Table striped bordered hover className="text-center">
                ...
                <tbody>
                    {
                        props.listaCamareiros.map((camareiros) => {
                            let data = new Date(camareiros.data_nasc)
                            return (
                                <tr key={camareiros.cpf_cam}>
                                    <td>{camareiros.cpf_cam}</td>
                                    <td>{camareiros.nome_cam}</td>
                                    <td>{data.toLocaleDateString()}</td>
                                    <td>{camareiros.endereco_cam}</td>
                                    <td>{camareiros.bairro}</td>
                                    <td>{camareiros.cidade_cam}</td>
                                    <td>{camareiros.uf_cam}</td>
                                    <td>{camareiros.nis}</td>
                                    <td>{camareiros.genero}</td>
                                    <td>
                                        <Button className="me-2" size="sm" variant="outline-primary" onClick={() => { props.exeAtualizacao(camareiros) }}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                                <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                                <path d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                            </svg>
                                        </Button>
                                        <Button className="" size="sm" variant="outline-danger" onClick={() => { props.exeRemocao(camareiros) }}>
