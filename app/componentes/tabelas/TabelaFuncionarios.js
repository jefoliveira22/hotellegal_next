...
export default function TabelaFuncionarios(props) {
    ...
    useEffect(() => { /* USEEFFECT EXECUTA AO RENDERIZAR A PAGINA BUSCANDO OS DADOS QUE VÃO EXIBIR NA TABELA */
        if (modoBusca) {
            fetch(ipBackend + 'funcionario/' + pesquisa.current.value,
                {
                    method: "GET"
                }).then((resposta) => {
            ...
            <Navbar expand="lg" className="mt-2 mb-2">
                ...
                <Navbar.Collapse id="navbarScroll">
                    <Nav className="me-auto my-2 my-lg-0" style={{ maxHeight: '100px' }} navbarScroll>
                        <Button variant="outline-success" onClick={() => { props.mudaFuncionario(FUNCIONARIOS.cadfuncionario) }} className="me-2">
                        ...
                        <Offcanvas show={show} onHide={handleClose} placement='bottom'>
                            <Offcanvas.Header closeButton>
                                <Offcanvas.Title>Lista de Funcionários</Offcanvas.Title>
                            </Offcanvas.Header>
                            <Offcanvas.Body>
                                - Esta tela oferece uma listagem dos funcionários cadastrados no hotel. <br />
                                - O botão "Adicionar", permite adicionar um novo funcionário ao sistema. <br />
                                - Na listagem, é possível alterar os dados, além de remove-lo, utilizando os botões da coluna "Ações". <br />
                                - O campo de busca permite localizar um funcionário pelo seu nome. <br />
                                - O cargo atribuido é seu nível de permissão no sistema. 
                            </Offcanvas.Body>
                        </Offcanvas>
                    </Nav>
