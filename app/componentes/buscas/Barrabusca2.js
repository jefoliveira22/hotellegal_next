...
export default function BarraBusca2({ placeHolder, dados, campoChave, campoBusca, funcaoSelecao, valor }) {
    ...
    function filtrarResultado() {
        setDadosLista(dados.filter((item) => {
            return termoBusca.length > 1 ? item.reserva.acomodacao.toLowerCase().includes(termoBusca.toLowerCase()) : false
        }));
                ...
                <ul data-resultado>
                    {
                        dadosLista.map(item => {
                            return <li key={item[campoChave]}
                                onClick={() => {
                                    setTermoBusca(item.reserva.acomodacao);
