...
export default function BarraBuscaREV({ placeHolder, dados, campoChave, campoBusca, funcaoSelecao, valor }) {
    ...
    function filtrarResultado() {
        setDadosLista(dados.filter((item) => {
            return termoBusca.length > 1 ? item["usuario"][campoBusca].toLowerCase().includes(termoBusca.toLowerCase()) : false
        }));
                ...
                <ul data-resultado>
                    {
                        dadosLista.map(item => {
                            return <li key={item[campoChave]}
                                onClick={() => {
                                    setTermoBusca(item["usuario"][campoBusca]);
                                ...
                                {
                                    item[campoChave] + '-' + item["usuario"][campoBusca]
                                }
                            </li>
                        })
                    }
